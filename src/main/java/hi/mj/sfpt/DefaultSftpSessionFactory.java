 ...
 */
@Component
public class DefaultSftpSessionFactory implements SessionFactory<SftpClient.DirEntry>,
    ...
    /**
     * The port over which the SFTP connection shall be established. If not specified, this value
     * defaults to <code>22</code>. If specified, this properties must be a positive number.
     *
     * @param port The port.
     * @see SshClient#connect(String, String, int)
     */
     ...
     * {@link DefaultSftpSessionFactory#setPrivateKey(Resource) privateKey} is mandatory.
     *
    ...
    /**
     * Provide a {@link HostConfigEntry} as an alternative for the user/host/port options. Can be
     * configured with a proxy jump property.
     *
    ...
    /**
     * Specifies a {@link Resource} that will be used for a host key repository. The data has to
     * have the same format as OpenSSH's known_hosts file.
     *
    ...
    /**
     * Allows you to set a {@link Resource}, which represents the location of the private key used
     * for authenticating against the remote host. If the privateKey is not provided, then the
     * {@link DefaultSftpSessionFactory#setPassword(String) password} property is mandatory.
     *
     ...
     * changes. As Spring Integration will not normally allow user interaction, the implementation
     * must respond to SSH protocol calls in a suitable way.
     *
     ...
     * {@link #setKnownHostsResource(Resource) knownHosts}). Default false (since 4.2). Set to true
     * if a knownHosts file is not provided.
     *
    ...
    /**
     * The timeout property is used as the socket timeout parameter, as well as the default
     * connection timeout. Defaults to <code>0</code>, which means, that no timeout will occur.
     *
    ...
    public SftpSession getSession() {
        ...
        try {
            ...
            if(sftpClient == null || !sftpClient.isOpen()) {
                sftpClient = SftpClientFactory.instance().createSftpClient(initClientSession(),
    ...
    private ClientSession initClientSession() throws IOException {
        ...
        if(config == null) {
            config = new HostConfigEntry(
                SshdSocketAddress.isIPv6Address(this.host) ? "" : this.host, this.host, this.port,
                this.user);
        }
        ClientSession clientSession = this.sshClient.connect(config).verify(verifyTimeout)
                                                    .getSession();
    ...
    private void doInitClient() throws IOException {
        ...
        ServerKeyVerifier serverKeyVerifier =
            this.allowUnknownKeys ? AcceptAllServerKeyVerifier.INSTANCE
        ...
        this.sshClient.setPasswordIdentityProvider(
            PasswordIdentityProvider.wrapPasswords(this.password));
        if(this.privateKey != null) {
            IoResource<Resource> privateKeyResource = new AbstractIoResource<Resource>(
                Resource.class, this.privateKey) {
            ...
            try {
                Collection<KeyPair> keys = SecurityUtils.getKeyPairResourceParser()
                                                        .loadKeyPairs(null, privateKeyResource,
                                                            FilePasswordProvider.of(
                                                                this.privateKeyPassphrase));
                this.sshClient.setKeyIdentityProvider(KeyIdentityProvider.wrapKeyPairs(keys));
            } catch(GeneralSecurityException ex) {
                throw new IOException("Cannot load private key: " + this.privateKey.getFilename(),
                    ex);
            }
        }
